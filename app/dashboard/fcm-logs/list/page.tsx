...
function FcmLogsListPageContent() {
  ...
  const [deviceFilter, setDeviceFilter] = useState(searchParams.get("device_id") || "all")
  ...
  useEffect(() => {
    const fetchFcmLogs = async () => {
      ...
      try {
        ...
        const endpoint = `payments/fcm-logs/?${query}`
          ...
          <CardHeader className="border-b border-gray-100 dark:border-strokedark">
            <CardTitle className="flex items-center space-x-2">
              <div className="p-2 bg-meta-2 dark:bg-orange-900 rounded-lg">
                <Bell className="h-5 w-5 text-primary dark:text-secondary" />
              </div>
            ...
            {loading ? (
              <div className="flex items-center justify-center py-6 sm:py-10">
                <div className="flex flex-col items-center space-y-4">
                  ...
                  <span className="text-body dark:text-bodydark">Chargement des journaux FCM...</span>
                </div>
              </div>
            ) : error ? (
                  ...
                  <TableBody>
                    {paginationData.results.map((log) => (
                      <TableRow key={log.id || log.uid} className="hover:bg-gray-50 dark:hover:bg-boxdark-2/50">
                        <TableCell data-label="Appareil">
                          <div className="flex items-center space-x-2">
                            <Smartphone className="h-4 w-4 text-bodydark2" />
                        ...
                        <TableCell data-label="Message">
                          <div className="max-w-md">
                            ...
                            {log.data && (
                              <div className="text-xs text-body dark:text-bodydark2 mt-1 break-words">
        ...
        {!loading && !error && paginationData.results.length === 0 && (
          <Card className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark mt-6">
            <CardContent className="p-12 text-center">
              <Bell className="h-12 w-12 text-bodydark2 mx-auto mb-4" />
